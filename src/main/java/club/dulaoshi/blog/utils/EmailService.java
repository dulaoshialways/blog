...
public class EmailService {
    ...
    @Value("${blog.email.myEmailAccount}")
    private String myEmailAccount;
    @Value("${blog.email.myEmailPassword}")
    private String myEmailPassword;
    @Value("${blog.email.myEmailSMTPHost}")
    private String myEmailSMTPHost;


    public void sendMail(String receiveMailAccount,String mailContent) throws Exception{
        ...
        // 3. 创建一封邮件
        MimeMessage message = createMimeMessage(session, myEmailAccount, receiveMailAccount,mailContent);
    ...
    private static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail,String mailContent) throws Exception {
        // 1. 创建邮件对象
        MimeMessage message = new MimeMessage(session);

        // 2. From: 发件人
        message.setFrom(new InternetAddress(sendMail, "杜老湿blog系统提示", "UTF-8"));

        // 3. To: 收件人（可以增加多个收件人、抄送、密送）
        message.addRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, receiveMail, "UTF-8"));

        // 4. Subject: 邮件主题
        message.setSubject("系统提示：请后台审核评论", "UTF-8");
